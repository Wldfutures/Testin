...
class Words:
    ...
    def load_words(self):
        ...
        import os
        word_files_path = os.path.join(os.path.dirname(__file__), "word_files", f"{file_name}.txt")
        with open(word_files_path, 'r') as file:
    ...
    def set_seed(self, seed):
        """Set random seed for reproducible word selection"""
        random.seed(seed)

    def get_letter_frequencies(self):
        """Calculate letter frequencies in remaining valid words"""
        freqs = {}
        total = len(self.words_list)
        for word in self.words_list:
            for c in word:
