...
class Wordle:
    ...
    def __init__(self, headless=False):
        ...
        if not headless:
            ...
            self.word_api = None
            self.get_from_db()
            # Initialize buttons as a 2D list without GUI
            self.buttons = [[{"text": "", "bg": self.BG} for _ in range(self.word_size)] for _ in range(6)]
            # Initialize keypad buttons in headless mode
            self.keypad_buttons = [[], [], []]
            for i in range(3):
                for j in range(10 if i > 0 else 6):
                    self.keypad_buttons[i].append({"bg": self.BG})
    ...
    def show_popup(self):
        if self.headless:
            if not self.won:
                print(f"Game Over! The word was {self.word_api.word}")
            self.reset()
        else:
            popup = tk.Toplevel()
